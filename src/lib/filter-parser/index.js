...
const toSQL = (filter) => { // 替换~为相应的操作符
    ...
    filter = filter.replace(/datetime'([^']+)'/gi, (text, s1) => {
        return `'${
            moment(s1, 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss')
        }'`;
    });
...
const toLodash = (filter) => {
    ...
    filter = filter.replace(/([^(~)]+)~eq~datetime'([^']+)'/gi, (text, field, value) => { // return moment(s1,'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss');
